...
void PongMode::update(float elapsed) {
	...
	static std::mt19937 rand; // mersenne twister pseudo-random number generator
	...
	{ // right player ai:
		...
		if (ai_offset_update < elapsed) {
			...
			ai_offset_update = (rand() / float(rand.max())) * 0.5f + 0.5f;
			ai_offset = (rand() / float(rand.max())) * 2.5f - 1.25f;
		}
